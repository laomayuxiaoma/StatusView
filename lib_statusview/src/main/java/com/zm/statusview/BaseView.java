...
public abstract class BaseView<T, VB extends ViewDataBinding> implements LifecycleObserver {
    ...
    protected void updateData(Object data) {
        ...
        if (baseView != null){
            baseView.handleCombinationView();
        }
    }
    ...
    private void handleCombinationView() {
        try {
            ...
            baseView = ConvertIntoView.convertIntoView(group, root.getContext(), conbinationBuilder.getTag(), conbinationBuilder.getViewConfigure(), data);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    protected void setViewConfigure(HashMap<String, Class> viewConfigure) {
        this.viewConfigure = viewConfigure;
    }

    //获取下级组合配置
    protected void getCombinationBuild() {
